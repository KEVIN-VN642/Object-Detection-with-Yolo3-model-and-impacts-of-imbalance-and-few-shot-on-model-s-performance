...
import copy
import numpy as np
from tensorflow.keras.utils import Sequence #new code
...
class BatchGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        base_grid_h, base_grid_w = net_h//self.downsample, net_w//self.downsample #grid size
        ...
        if r_bound > len(self.instances):
            r_bound = len(self.instances)   #constraint r_boundd at len(self.instances)
            l_bound = r_bound - self.batch_size # update new l_bound since r_bound has changed

        x_batch = np.zeros((r_bound - l_bound, net_h, net_w, 3))             # one batche of input images
    ...
    def _get_net_size(self, idx):
        if idx%10 == 0:
            net_size = self.downsample*np.random.randint(self.min_net_size/self.downsample, \
                                                         self.max_net_size/self.downsample+1)      #get new image size every 10 batches
    ...
    def _aug_image(self, instance, net_h, net_w):
        ...
        new_ar = (image_w + np.random.uniform(-dw, dw)) / (image_h + np.random.uniform(-dh, dh)); #new anchor ratio
        scale = np.random.uniform(self.aug_scale[0], self.aug_scale[1]); 
        
        #determine new height and width of image base on scale and new anchor ratio
        if (new_ar < 1):
        ...
        im_sized = random_distort_image(im_sized,hue=self.color_distort[0], saturation=self.color_distort[1],exposure=self.color_distort[2])
